...
from llmga.diffusers.pipeline_stable_diffusion_inpaint_lpw import StableDiffusionInpaintPipeline
import os
import copy
import numpy as np
...
def resize2512(init_image,mask_image):
    l=512
    width=init_image.width
    height=init_image.height
    if height>width:
        mask_image=mask_image.resize((l, int(height/width*l)),Image.ANTIALIAS)
        init_image=init_image.resize((l, int(height/width*l)),Image.ANTIALIAS)
    else: 
        mask_image=mask_image.resize((int(width/height*l), l),Image.ANTIALIAS)
        init_image=init_image.resize((int(width/height*l), l),Image.ANTIALIAS)
    mask_np = np.array(mask_image)
    mask_np[mask_np<125]=0
    mask_np[mask_np>=125]=255
...
def main(args):
    ...
    while True:
        ...
        if id1!=-1:
            resized_image, resized_mask=resize2512(init_image,mask)
            img = pipe(prompt=caption, image=resized_image, mask_image=resized_mask).images[0]
