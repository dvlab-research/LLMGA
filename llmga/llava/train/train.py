...
class LazySupervisedDataset(Dataset):
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        if not has_img:
            ...
            if self.data_args.mm_use_output_start_end and is_generation:
                tp["conversations"][1]["value"] = DEFAULT_OUTPUT_START_TOKEN + " " + tp["conversations"][1][
                    "value"] + " " + DEFAULT_OUTPUT_END_TOKEN
        ...
        if 'image' in sources[0]:
            ...
            if txn is not None:
                image = txn(image_file).convert('RGB')
            else:
            ...
            if i >= self.len1 and i < self.len1 + self.len2:
                image = np.array(image)
                mask, is_outpaint = self.gen_mask(image, i)
                mask[mask < 0.5] = 0
                mask[mask >= 0.5] = 1
                image = (1 - mask) * image
                ...
                if is_outpaint:
                    sources[0]["conversations"][0]["value"] = DEFAULT_IMAGE_TOKEN + "\n" + random.choice(outpaint_prompt)
                    if self.data_args.mm_use_output_start_end:
                        sources[0]["conversations"][1]["value"] = DEFAULT_OUTPUT_START_TOKEN + " " + \
                                                                  sources[0]["conversations"][1][
                                                                      "value"] + " " + DEFAULT_OUTPUT_END_TOKEN
                else:
                    sources[0]["conversations"][0]["value"] = DEFAULT_IMAGE_TOKEN+ "\n" + random.choice(inpaint_prompt)
                    if self.data_args.mm_use_output_start_end:
                        sources[0]["conversations"][1]["value"] = DEFAULT_OUTPUT_START_TOKEN + " " + \
                                                                  sources[0]["conversations"][1][
                                                                      "value"] + " " + DEFAULT_OUTPUT_END_TOKEN
...
def make_supervised_data_module(tokenizer: transformers.PreTrainedTokenizer,
    ...
    train_dataset = LazySupervisedDataset(tokenizer=tokenizer,
                                          ...
                                          data_path3=data_args.data_path3,
                                          data_args=data_args)
...
def train():
    ...
    if model_args.vision_tower is not None:
        ...
        model.config.mm_use_output_start_end = data_args.mm_use_output_start_end = model_args.mm_use_output_start_end
        training_args.use_output_start_end = model_args.mm_use_output_start_end
