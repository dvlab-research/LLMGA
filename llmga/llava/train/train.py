...
from llmga.llava.datasets.vqa_dataset import VQADataset
from llmga.llava.datasets.inpainting_dataset import InpaintingDataset
from llmga.llava.datasets.sg_dataset import SGDataset
from llmga.llava.datasets.edit_dataset import EditDataset
from llmga.llava.datasets.text_dataset import TextDataset
local_rank = None
...
class ModelArguments:
    ...
    mm_use_output_start_end: bool = field(default=False)
...
def make_supervised_data_module(tokenizer: transformers.PreTrainedTokenizer,
    ...
    vqadataset=VQADataset(tokenizer=tokenizer,
               data_path=data_args.data_path,
               data_args=data_args)
    inpaintingdataset=InpaintingDataset(tokenizer=tokenizer,
            data_path=os.path.join(data_args.data_path2,"inpainting","train.json"),
            data_args=data_args)
    sgdataset=SGDataset(tokenizer=tokenizer,
            data_path=os.path.join(data_args.data_path2,"SG","train.json"),
            data_args=data_args)
    editdataset=EditDataset(tokenizer=tokenizer,
            data_path=os.path.join(data_args.data_path2,"Edit","train.json"),
            data_args=data_args)
    textdataset=TextDataset(tokenizer=tokenizer,
            data_path=data_args.data_path3,
            data_path2=os.path.join(data_args.data_path2,"T2I","train.json"),
            data_args=data_args)    
    train_dataset = ConcatDataset([vqadataset, inpaintingdataset, sgdataset, editdataset, textdataset])
...
def train(attn_implementation=None):
    ...
    elif "phi_3" in model_args.version:        
        if model_args.version in conversation_lib.conv_templates:
            conversation_lib.default_conversation = conversation_lib.conv_templates[model_args.version]
        else:
            conversation_lib.default_conversation = conversation_lib.conv_templates["phi_3"]
    elif "qwen_2" in model_args.version:        
        if model_args.version in conversation_lib.conv_templates:
            conversation_lib.default_conversation = conversation_lib.conv_templates[model_args.version]
        else:
