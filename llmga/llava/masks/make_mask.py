...
# from masks.segmask.SegmentationMask import SegmentationMask
...
class RandomIrregularMaskGenerator:
    ...
    def __call__(self, img, iter_i=None, raw_image=None):
        ...
        return make_random_irregular_mask(img.shape[:2], max_angle=self.max_angle, max_len=cur_max_len,
...
class RandomRectangleMaskGenerator:
    ...
    def __call__(self, img, iter_i=None, raw_image=None):
        ...
        return make_random_rectangle_mask(img.shape[:2], margin=self.margin, bbox_min_size=self.bbox_min_size,
...
class RandomSuperresMaskGenerator:
    ...
    def __call__(self, img, iter_i=None):
        return make_random_superres_mask(img.shape[:2], **self.kwargs)
...
class MixedMaskGenerator:
    ...
    def __call__(self, img, iter_i=None, raw_image=None):
        ...
        if self.invert_proba > 0 and random.random() < self.invert_proba:
            result = 1 - result
        return result, kind==(len(self.probas)-1)
...
def get_mask_generator(kind=None, kwargs=None):
    if kind is None:
        kind = "mixed"
    if kwargs is None:
        kwargs = {
...
if __name__ == '__main__':
    ...
    import numpy as np
    import os
    save_path ="/mnt/bn/xiabinpaint/CVPR2024v4/masks/gen_masks"
    os.makedirs(save_path,exist_ok=True)
    gen_mask=get_mask_generator()
    tp=np.zeros([512, 512, 3], dtype=int)
    for i in enumerate(100):
        mask,_=gen_mask(tp,i)
