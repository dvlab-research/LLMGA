...
class InpaintingTextTrainDataset(Dataset):
    def __init__(self, indir,indir2, args=None, mask_gen_kwargs=None):
    ...
    "outpainting_kwargs": {
        ...
        indir2 = os.path.join(indir2, "train.json")
    ...
    def __getitem__(self, index):
        if index<self.len_1+self.len_2:
            ...
            info_json =  self.prompt_dict3[index-self.len_1-self.len_2]
            img=Image.open(os.path.join(self.image_folder2,info_json["image_id"])).convert("RGB")
            prompt = info_json["caption"]
        if random.random()<0.05:
            prompt=""
        img=self.train_transforms(img)
        mask = self.mask_generator(img, iter_i=index)
