...
class SemanticStableDiffusionImg2ImgPipeline_DPMSolver(DiffusionPipeline):
    ...
    def generate_mask(
        ...
        b, l, h= prompt_embeds.shape
        prompt_embeds = prompt_embeds.view(2,b//2,l,h)
        negative_prompt_embeds = negative_prompt_embeds.view(2,b//2,l,h)
        target_prompt_embeds=prompt_embeds[0]
        target_negative_prompt_embeds=negative_prompt_embeds[0]
        source_prompt_embeds=prompt_embeds[1]
        source_negative_prompt_embeds=negative_prompt_embeds[1]
    ...
    def __call__(
        ...
        mask_image = preprocess_mask(mask_image, self.batch_size)
        latent_height, latent_width = mask_image.shape[-2:]
        mask_image = torch.cat([mask_image] * num_images_per_prompt)
        mask_image = mask_image.to(device=self.device, dtype=latents.dtype)
        ...
        for i, t in enumerate(self.progress_bar(timesteps, verbose=verbose)):
            ...
            if do_classifier_free_guidance:
                ...
                noise_pred = noise_pred_uncond + noise_guidance * mask_image
